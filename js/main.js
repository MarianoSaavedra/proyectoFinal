...
formularioUno.addEventListener('submit', (e) => crearCuenta(e))
...
const crearCuenta = (e) => {
    ...
    if(!input[0].value || !input[1].value){
        error.innerHTML = `Completar todos los campos requeridos.`
    }
    else if(input[0].value.length < 5 || input[1].value.length < 5 ){
...
const ordenarProductos = () => {
        let acumula = "";
        let orden = document.getElementById("selectOrdenar").value;
...
const buscarProductos = (e) => {
    ...
    let acumula = "";
    products.filter(item => item.title.toLocaleLowerCase().includes(busqueda.toLocaleLowerCase())).map((item) =>
        acumula += `
        <div class="productos">
            <h2>${item.title}</h2>
            <img src="${item.thumbnail}" alt="${item.description}" class="imagenes">
